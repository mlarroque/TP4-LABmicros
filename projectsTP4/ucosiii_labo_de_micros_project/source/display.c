...
#define  OS_DISPLAY_ACTIVE 1
#if OS_DISPLAY_ACTIVE
#include <os.h>
static OS_Q * pDISPLAY2OSqueue;
static OS_ERR osDISPLAY_err;
#endif
...
void InitializeDisplay(void * p2userData)
{
	if(!initialized)
	{
		...
		if(OS_DISPLAY_ACTIVE)
		{
			pDISPLAY2OSqueue = (OS_Q *)p2userData;
		}
	}
}
...
void GenerateDisplayEv(void)
{
	...
	if(OS_DISPLAY_ACTIVE)
	{
		OSQPost(pDISPLAY2OSqueue, 1, 1, OS_OPT_POST_ALL, &osDISPLAY_err);
	}
