...
#if OS_ACTIVE
#include <os.h>
static OS_Q * pLECTOR2OSqueue;
static OS_ERR osLECTOR_err;
#endif
...
void hwLectorInit(void * p2resourcesData)
{
	if (!hwLectorIsInit)
	{
	...
	if(OS_ACTIVE)
	{
		pLECTOR2OSqueue = (OS_Q *)p2resourcesData;
	}
}
...
void enableRoutine(void)
{
	...
	else
	{
		...
		if(OS_ACTIVE)
		{
			OSQPost(pLECTOR2OSqueue, 1, 1, OS_OPT_POST_ALL, &osLECTOR_err);
		}
