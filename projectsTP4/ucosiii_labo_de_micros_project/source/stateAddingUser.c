		...
		case ENTER: // Selects current option
			...
			switch(ud->option)
			{
				...
				default: // number
					...
					if(j == ID_LENGTH){ // id entered and not taken, we ask for PIN
						if(!verifyID(ud->received_ID)){ // ID not taken
							nextState.name = ADDING_USER_PIN;
							nextState.routines[INPUT_EV] = &AUPinputEvHandler;
							nextState.routines[TIMER_EV] = &AUPtimerEvHandler;
							nextState.routines[KEYCARD_EV] = &AUPkeycardEvHandler;
