...
#define  OS_ENCODER_ACTIVE 1
#if OS_ENCODER_ACTIVE
#include <os.h>
static OS_Q * pENCODER2OSqueue;
static OS_ERR osENCODER_err;
#endif
...
void initializeEncoder(void * p2userData)
{
	if(!initialized_enc)
	{
		uint8_t i;
		...
		if(OS_ENCODER_ACTIVE)
		{
			pENCODER2OSqueue = (OS_Q *)p2userData;
		}
...
void rotationCallback(void)
{
	...
	if(event == COUNT_UP)
	{
		...
		if(OS_ENCODER_ACTIVE)
		{
			OSQPost(pENCODER2OSqueue, 1, 1, OS_OPT_POST_ALL, &osENCODER_err);
		}
	}
